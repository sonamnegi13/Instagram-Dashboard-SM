...
def validate_api_key(api_key: str) -> tuple[bool, str]:
    """
    Validate API key against HasData Instagram Profile API.
    Per docs: https://docs.hasdata.com/apis/instagram/profile
    Correct parameter name is 'handle' (NOT 'username' — that causes 422).
    """
    try:
        resp = requests.get(
            "https://api.hasdata.com/scrape/instagram/profile",
            headers={"x-api-key": api_key, "Content-Type": "application/json"},
            params={"handle": "instagram"},   # ← CORRECT per HasData docs
            timeout=15,
        )
        if resp.status_code == 200:
            return True, "Connected — API key valid and working"
        elif resp.status_code == 401:
            return False, "Invalid API key (401 Unauthorized)"
        elif resp.status_code == 403:
            return False, "API key has no access (403 Forbidden)"
        elif resp.status_code == 422:
            return False, "Request rejected (422) — contact HasData support"
