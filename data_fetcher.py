"""
data_fetcher.py — InstaLens v4.0
...
Only one endpoint exists. Posts come embedded in 'latestPosts' array.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
#  Custom exceptions — so callers can handle errors explicitly
# ─────────────────────────────────────────────────────────────────────────────
...
class HasDataFetcher:
    ...
    NEVER silently falls back to mock data — raises exceptions on failure
    so the caller can show clear error messages to the user.
    """
    ...
    def fetch_profile_raw(self, handle: str) -> dict:
        """
        Fetch raw profile JSON from HasData.
        Raises HasDataAuthError / HasDataRateLimitError / HasDataFetchError
        instead of silently returning None or mock data.
        """
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                resp = requests.get(
                    ...
                    params={"handle": handle},   # ← CORRECT param per HasData docs
                    timeout=30,
                )
                ...
                if resp.status_code == 200:
                    ...
                    raise HasDataAuthError(
                        ...
                        "Check your HasData API key."
                    )

                elif resp.status_code == 422:
                    raise HasDataFetchError(
                        f"Invalid request for @{handle} (422). "
                        "Verify the Instagram handle exists and is public."
                    )
                    ...
                    if attempt < self.MAX_RETRIES - 1:
                        ...
                        time.sleep(wait)
                        continue
                    raise HasDataRateLimitError(
                        "Rate limit exceeded (429). Wait a minute and try again."
                    )

                elif resp.status_code == 404:
                    raise HasDataFetchError(
                        f"Profile @{handle} not found (404). "
                        "Check the username spelling."
                    )

                else:
                    raise HasDataFetchError(
                        f"HasData API error {resp.status_code} for @{handle}."
                    )

            except (HasDataAuthError, HasDataRateLimitError, HasDataFetchError):
                raise   # re-raise — don't retry auth/validation errors
                ...
                last_error = "Cannot reach api.hasdata.com — check network"
                if attempt < self.MAX_RETRIES - 1:
                    time.sleep(self.RETRY_DELAY)
                    continue

            except requests.exceptions.RequestException as e:
                last_error = f"Network error: {str(e)[:120]}"
                break
    ...
    def fetch_all(
        ...
        """
        Fetch all profiles. Returns a dict with:
          {
            "profiles": { handle: transformed_data },
            "hashtags": { tag: engagement_data },
            "errors":   { handle: error_message },   ← NEW — explicit error map
            "source":   "live"
          }
        """
        ...
        for handle in usernames[:3]:
            try:
                ...
                # Auth error applies to all profiles — stop fetching
                errors[handle] = str(e)
                raise   # propagate up so caller can disable live mode
            except (HasDataFetchError, HasDataRateLimitError) as e:
                errors[handle] = str(e)
                # Continue with other profiles

        # Derive hashtag data from fetched post captions
        all_tags: set = set()
            ...
            for pdata in profiles.values():
                for post in pdata.get("_raw_posts", []):
                    if tag in (post.get("caption", "") or "").lower():
                        eng = post.get("likesCount", 0) + post.get("commentsCount", 0)
                        total_eng += eng
                        count += 1
    ...
    def _transform(self, handle: str, raw: dict, days: int) -> dict:
        """
        Map HasData profile JSON → dashboard data shape.

        HasData response fields (documented):
          biography, followersCount, followingCount, postsCount,
          isVerified, profilePicUrl, latestPosts[]
            └ each post: likesCount, commentsCount, timestamp,
                         caption, type, shortCode
        """
        followers   = raw.get("followersCount", 0) or 0
        following   = raw.get("followingCount", 0) or 0
        total_posts = raw.get("postsCount", 0) or 0
        ...
        # Filter to date window; fall back to all if window is too narrow
        cutoff = datetime.now() - timedelta(days=days)
        recent = []
        for p in all_posts:
            try:
                ts = p.get("timestamp", 0) or 0
                if ts and datetime.fromtimestamp(float(ts)) >= cutoff:
                    recent.append(p)
            except Exception:
                pass
        if not recent:
        ...
        def _avg(lst: list, key: str) -> int:
            vals = [x.get(key, 0) or 0 for x in lst]
        ...
        reel_lk     = _avg(reels, "likesCount")
        reel_cm     = _avg(reels, "commentsCount")
        reel_er     = round(((reel_lk + reel_cm) / max(followers, 1)) * 100, 2) if reels else round(er * 1.3, 2)
...
# ─────────────────────────────────────────────────────────────────────────────
#  Mock / Demo Data  — explicit, clearly labelled
# ─────────────────────────────────────────────────────────────────────────────
...
def generate_mock_profile(username: str, days: int) -> dict:
    ...
    return {
        "username":          username,
        "bio":               f"Demo profile — this is mock data for @{username}.",
...
def _build_engagement_series(posts: List[dict], days: int, avg_saves: int) -> List[dict]:
    ...
    for p in posts:
        try:
            d = datetime.fromtimestamp(p.get("timestamp", 0) or 0).strftime("%b %d")
            by_date[d]["likes"]    += p.get("likesCount", 0) or 0
            by_date[d]["comments"] += p.get("commentsCount", 0) or 0
            by_date[d]["saves"]    += int((p.get("likesCount", 0) or 0) * 0.08)
        except Exception:
            pass
    if not by_date:
...
def _generate_heatmap(posts: List[dict]) -> List[List[float]]:
    ...
    for p in posts:
        try:
            dt   = datetime.fromtimestamp(p.get("timestamp", 0) or 0)
            dow  = dt.weekday()
...
def _build_top_posts(posts: List[dict], followers: int) -> List[dict]:
    top    = sorted(posts, key=lambda p: p.get("likesCount", 0) or 0, reverse=True)[:6]
    result = []
    for p in top:
        lk = p.get("likesCount", 0) or 0
        cm = p.get("commentsCount", 0) or 0
        ...
        result.append({
            ...
            "date":            (datetime.fromtimestamp(p["timestamp"]).strftime("%b %d")
...
def _extract_hashtag_engagement(posts: List[dict]) -> List[dict]:
    ...
    return sorted([{"tag": t, "avg_engagement": int(sum(v) / len(v))}
                   for t, v in tag_eng.items() if v],
                  key=lambda x: x["avg_engagement"], reverse=True)[:10]
